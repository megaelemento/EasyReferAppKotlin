...
class AdminDriverSocket(private val token: String) {
    ...
    private fun _connect() {
        ...
        webSocket = client.newWebSocket(request, object : WebSocketListener() {
            override fun onOpen(webSocket: WebSocket, response: Response) {
                android.util.Log.d("AdminDriverSocket", "Conectado al servidor")
            }
            ...
            override fun onClosed(webSocket: WebSocket, code: Int, reason: String) {
                android.util.Log.w("AdminDriverSocket", "Cerrado: $code $reason")
                scheduleReconnect()
            }
    ...
    private fun scheduleReconnect() {
        if (isStopped) return
        android.util.Log.d("AdminDriverSocket", "Reconectando en 3s...")
